...
// 2. Modals destruct (use regex to be safe)
text = text.replace(/const \{\s*isDrawerOpen,\s*editingLesson,\s*drawerInitialTab,\s*handleOpenDrawer,\s*handleCloseDrawer,/, 'const {');

// 3. Dnd Hook (use regex to be safe)
text = text.replace(/\/\/ ── Drag & Drop ────────────────────────────────────────────────────────[\s\S]*?\} = useScheduleDragAndDrop\([\s\S]*?\);\n/, "");
...
// 7. DndContext start & end (using regex)
text = text.replace(/<DndContext[\s\S]*?onDragCancel=\{[\s\S]*?\}\s*>/, "");
text = text.replace(/\{createPortal\([\s\S]*?<\/DragOverlay>,\s*document\.body\s*\)\}\s*<\/DndContext>/, "");

// 8. MonthView update
text = text.replace(/<MonthView[\s\S]*?\/>/, `<MonthView
...
// 9. Drawer to Context Panel (using regex to ignore internal Drawer changes)
text = text.replace(/\{\/\* Drawer урока \*\/\}\s*<LessonDrawer[\s\S]*?\/>/, `          </div>
...
// 10. Podskazka
text = text.replace(/\{\/\* Подсказка drag-and-drop \*\/\}\s*\{view === "week" && \([\s\S]*?\)\}/, "");

fs.writeFileSync("src/pages/SchedulePage.jsx", text);
console.log("Done patching flawlessly.");
