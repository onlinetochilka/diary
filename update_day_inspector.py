...
import { Select, Checkbox } from '../ui/index.js';
  ...
  return (
    <div className="px-4 py-2 space-y-2 border-b border-stone-100/50 pb-4 mb-2">
      <p className="text-[10px] font-bold tracking-widest text-stone-400 uppercase mb-1.5 flex items-center gap-1.5">
        <span className="w-1.5 h-1.5 rounded-full bg-[#006584]" />
        Статус урока
      </p>
        ...
        {STATUS_OPTIONS.map(opt => (
          <button
            ...
            className={cn(
              ...
              (formData.status || 'planned') === opt.value
...
function PaymentBlock({ formData, onPatch }) {
  // Hide for groups since payment might be per student there
      ...
      <div className="flex items-center gap-2 mb-2">
        <Checkbox 
          ...
          onCheckedChange={(checked) => {
            onPatch({ hwAssigned: !checked, homework: checked ? '' : localHw });
          }} 
        />
            ...
            <div className="space-y-2 mt-3">
              ...
              <div className="grid grid-cols-1 gap-2">
                {groupStudents.map(st => {
                  ...
                  const hwStatus = isDone ? (formData.hwStatuses?.[st.id] || 'on_time') : 'not_done';
                  return (
                    <div key={st.id} className="flex items-center justify-between p-2 rounded-xl border border-stone-100 bg-stone-50/50">
                      ...
                      <div className="flex bg-stone-100 p-0.5 rounded-lg shrink-0">
                        ...
                        }} className={cn("px-2.5 py-1 text-[10px] font-bold rounded-md transition-colors", hwStatus === 'not_done' ? 'bg-white text-stone-700 shadow-sm' : 'text-stone-500 hover:text-stone-700')}>Не выполнено</button>
                        <button onClick={() => {
...
"""
    with open('rewrite_day_inspector.js', 'w', encoding='utf-8') as f:
        f.write(f"const fs = require('fs');\nfs.writeFileSync('src/components/schedule/DayInspector.jsx', `{new_code}`);")
